"""Text editor module for creating and editing documents using the Thing Explainer word list."""
...
class SimpleTextEditor(App):
    ...
    def __init__(self, output_path: Path, **kwargs):
        ...
        word_forms_path = Path(__file__).parent / "thing-explainer" / "word-forms.json"
        word_forms: dict[str, dict[str, str]] = json.loads(word_forms_path.read_text())
...
def run_editor(output_path: Path) -> None:
    """Run the text editor application with the specified output path.
    
    Args:
        output_path: Path to the directory where files will be saved.
    """
    app = SimpleTextEditor(output_path)
    app.run()
